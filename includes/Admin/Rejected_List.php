 ...
 */
class Rejected_List extends \WP_List_Table
{
    ...
    public function get_columns()
    {
        return [
            ...
            'rejected_by' => __('Rejected By', 'nas-academy')
        ];
    }
    ...
    public function column_name($item){
        ...
        $actions['delete'] = sprintf( '<a href="%s" class="submitdelete" onclick="return confirm(\'Are you sure?\');" title="%s">%s</a>', wp_nonce_url( admin_url( 'admin-post.php?action=wd-delete-participator&id=' . $item->id ), 'wd-delete-participator' ), $item->id, __( 'Delete', 'nas-academy' ), __( 'Delete', 'nas-academy' ) );
        $actions['restore'] = sprintf( '<a href="javascript:void(0);" class="nas-restore-reject" id="%s" data-phone="%s">%s</a>', $item->id, $item->phone, __( 'Restore', 'nas-academy' ), __( 'Restore', 'nas-academy' ) );
    ...
    public function prepare_items()
    {
        ...
        $this->items = wd_get_rejected_participators($args);
        $this->set_pagination_args([
            'total_items' => wd_rejected_participators_count(),
            'per_page'   => $per_page
        ]);
    }
}
